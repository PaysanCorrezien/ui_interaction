...
impl WindowsElement {
    ...
    fn is_input_control(&self) -> Result<bool, Box<dyn Error>> {
        ...
        Ok(match control_type {
            ...
            ControlType::Custom => self.element.get_pattern::<UIValuePattern>().is_ok(),
            _ => false,
        })
    }
}
        ...
        match self.get_text() {
            Ok(current_text) => {
                if current_text == text {
                    ...
                    warn!(
                        "Text was not set correctly. Expected: {}, Got: {}",
                        text, current_text
                    );
                    ...
                    match self.get_text() {
                        Ok(retry_text) => {
                            if retry_text == text {
                                ...
                                warn!(
                                    "Text still not set correctly on retry. Expected: {}, Got: {}",
                                    text, retry_text
                                );
                            }
                        }
